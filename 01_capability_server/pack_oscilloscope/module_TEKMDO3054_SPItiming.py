...
def interface_set_zoom(instrName, zoom_position, zoom_scale):
    ...
    """
    try:
        zoom_position = float(zoom_position)
...
if __name__ == '__main__':
    instrName = "TCPIP::" + "169.254.188.192" + "::INSTR"
    ...
    interface_close_ch(instrName, "CH3")
    # 设置时基
    interface_set_horizontal_scale(instrName, 40E-9)
    # 调用set_trigger函数设置触发类型
    interface_set_trigger(instrName, "FALL", "CH4", 1.42, 400E-9)
    # 设置为单次触发
    interface_set_trigger_mode(instrName, "SEQUENCE")
    time.sleep(20)
    # 搜索通道4标记点的位置坐标,调用search_position函数实现
    mark_position1 = interface_set_search(instrName, "CH4", "RISE", 1.26)
    # 搜索通道2标记点的位置坐标
    mark_position2 = interface_set_search(instrName,  "CH2", "RISE", 1.26)
    # 调用timing_position函数获得光标a和光标b的位置坐标
    position_return = interface_timing_position1(instrName, mark_position1['query'], mark_position2['query'])
    ...
    cursor_return = interface_set_cursor(instrName, "CH2", 5.26, 1.26, position_a, position_b)
    # 保存图片
    delta = position_return['min_delta']
